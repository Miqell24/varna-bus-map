...
// Varna: ONE feed from the Bulgarian National Access Point — 36 bus lines and
// the four surviving trolleybus lines (82, 83, 86, 88), which ride the road
// graph in the family's green. No tram: Varna has never had one.
...
// Exact-match repairs for poles a feed names badly. Varna's problem is not a
// handful of poles but the whole file — see varnaName and lib/caps.mjs.
const NAME_FIX = {};
...
//
// The Bulgarian NAP (sipbg.gov.bg) publishes Varna's timetable as a single
// nightly GTFS with shapes. route_type 11 marks the trolleybuses, which the
// engine already knows to paint green.
//
// STOP NAMES are this feed's real work. 606 of its 616 names shout, and each
// one carries its own transliteration behind a slash:
//   "ЦЕНТРАЛНА ПОЩА / АПТ. САНИТА / TSENTRALNA POSHTA (SANITA)"
// The Latin half is a second copy of the same name, not part of it, so it goes;
// then lib/caps.mjs rewrites the shouted Cyrillic through a dictionary
// harvested from the OSM extract this build already reads (the Athens recipe,
// retold in Bulgarian — Bulgarian capitalises only the first word of a name,
// so the dictionary, not title case, has to decide).
import { buildNameDict, cyrillicTitleCase } from './lib/caps.mjs';

const LBL = new Map();
const isRailTrunk = () => false;

// Initialisms the city shouts on the pole too, so the rewrite leaves them.
const CAPS_ACRONYMS = new Set(['ЖП', 'КК', 'ДКЦ', 'МБАЛ', 'ТЕЦ', 'ОУ', 'СУ', 'ВВМУ',
  'ТУ', 'ИУ', 'МУ', 'ПГ', 'СОУ', 'АМ', 'БГ', 'КАТ', 'РУМ', 'ЦУМ', 'ХЕИ']);
const nameDict = (() => {
  const docs = [];
  for (const f of ['data/osm/tiles/t1.json', 'data/osm/tiles/t2.json',
                   'data/osm/tiles/t3.json', 'data/osm/tiles/t4.json']) {
    const p = join(ROOT, f);
...
// A Varna stop name is a slash-separated stack of three different things:
//   "ЯНКО МИХАЙЛОВ / КЪМ ЦАР ОСВОБОДИТЕЛ / YANKO MIHAYLOV"
//   name            direction              transliteration
// Only the first is the name. The transliteration is a second copy of it and
// goes; so does the direction member (488 of them, opening with ЗА / КЪМ /
// ОТ), which belongs to the pole, not to the place — the map aggregates poles
// by name and prints that name on the street, exactly as in Niš.
// NB: no \b here — JavaScript's word boundary is defined on ASCII letters
// only, so /^ЗА\b/ never matches "ЗА ЦЕНТЪР" and every direction member
// survived the first run.
const DIRECTION = /^(ЗА|КЪМ|ОТ)(\s|$)/;
const varnaName = (n) => {
  const parts = n.split('/').map((s) => s.trim()).filter(Boolean);
  const cyr = parts.filter((s) => /[А-Яа-яЁё]/.test(s) && !DIRECTION.test(s));
  const kept = (cyr.length ? cyr : parts).join(' / ').replace(/\s+/g, ' ').trim();
  return cyrillicTitleCase(kept, nameDict, CAPS_ACRONYMS);
};

const MODES = [{
  mode: 'bus', label: 'buses & trolleybuses', graphMode: 'road',
  // the city, Zlatni Pyasatsi and the villages inland — 2 × 2 tiles
  osmFiles: Array.from({ length: 4 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['409']),
  feeds: [
    { tag: 'vn', dir: 'data/gtfs', routeTypes: ['3', '11'],
